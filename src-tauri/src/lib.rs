...
#[tauri::command]
async fn pause_media_download_task(
    app: AppHandle,
    store: State<'_, TaskStore>,
    id: String,
) -> Result<(), String> {
...
#[tauri::command]
async fn resume_media_download_task(
    app: AppHandle,
    client: State<'_, AppHttpClient>,
    store: State<'_, TaskStore>,
    id: String,
...
#[tauri::command]
async fn retry_media_download_task(
    app: AppHandle,
    client: State<'_, AppHttpClient>,
    store: State<'_, TaskStore>,
    id: String,
...
#[tauri::command]
async fn cancel_media_download_task(
    app: AppHandle,
    store: State<'_, TaskStore>,
    id: String,
) -> Result<(), String> {
...
#[tauri::command]
async fn show_file_in_folder(app: AppHandle, path: String) -> Result<(), String> {
    // 1. Resolve full path (if 'path' is just filename) or use as is
    // Assuming 'path' passed from frontend is the 'save_path' which is just filename in the struct
    // We need to prepend download_dir
    let settings = load_settings(&app);
    let download_dir = PathBuf::from(settings.download_path.unwrap_or_default());
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            pause_media_download_task,
            resume_media_download_task,
            retry_media_download_task,
            cancel_media_download_task,
            show_file_in_folder,
