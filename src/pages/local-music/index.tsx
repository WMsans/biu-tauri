...
const LocalMusicPage = () => {
  ...
  useEffect(() => {
    const init = async () => {
      if (localDirs?.length) {
        const data = await tauriAdapter.scanLocalMusic(localDirs);
        setList(data);
      } else {
        setList([]);
      }
  ...
  const addDirectory = async () => {
    const dir = await tauriAdapter.selectDirectory();
    if (!dir) return;
  ...
  const openFile = async (filePath: string) => {
    await tauriAdapter.showFileInFolder(filePath);
  };
  ...
  const deleteFile = (filePath: string) => {
    onOpenConfirmModal({
      ...
      onConfirm: async () => {
        const ok = await tauriAdapter.deleteLocalMusicFile(filePath);
        if (ok) {
